...
An animation binding requires `id`, existing `elementId`, existing `behaviorId`, `falloffIds`, channels, and role. Valid channels are `x`, `y`, `rotation`, `scale`, `opacity`, `color`, `letterSpacing`, `lineHeight`, `cornerRadius`, `width`, `height`, and `pathProgress`. A scene permits at most one `primary` and two `supporting` bindings.
