...
export function fitTextInCircle(text: string, options: { radius: number; fontSize: number; inset: number; lineHeight: number }) {
  const capacity = Math.max(1, Math.floor((options.radius - options.inset) * 2 / options.fontSize));
  const characters = Array.from(text);
  const lines = Array.from({ length: Math.ceil(characters.length / capacity) }, (_, index) => characters.slice(index * capacity, (index + 1) * capacity).join(''));
  return {
    text: lines.join('\n'),
